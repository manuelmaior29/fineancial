...
def main():
    ...
    if st.button("Clear All Data"):
        st.session_state.data_transactions = pd.DataFrame()
        st.info("Cleared all transaction data.")
    ...
    if csv_files is not None:
        ...
        # Append new uploads
        st.session_state.data_transactions = pd.concat([st.session_state.data_transactions, df_transactions])
        st.session_state.input_data_transactions += 1
    ...
    df_transactions_filtered = filters.filter_by_date_period(st.session_state.data_transactions, 
